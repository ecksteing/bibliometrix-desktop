...
# Used before uninstall/upgrade so R-Portable / library files are not locked.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\stop_bibliometrix.ps1
#   powershell -ExecutionPolicy Bypass -File .\stop_bibliometrix.ps1 -AppDir "C:\...\Bibliometrix Desktop"
...
$RProcessNames = @(
    "Rscript.exe", "Rgui.exe", "Rterm.exe", "R.exe",
    "Rscript", "Rgui", "Rterm", "R"
)
...
function Get-TargetProcessIds {
    ...
    Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        $isLauncher = $name -match '^run_bibliometrix(\.exe)?$'
        $isR = $RProcessNames -contains $name
        if (-not ($isLauncher -or $isR)) { return }
    ...
    Get-Process -Name "run_bibliometrix" -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    foreach ($port in $Ports) {
        ...
        foreach ($procId in @($ownerIds.Keys)) {
            ...
            $isOurs = (Test-UnderAppDir $proc.ExecutablePath) -or
                ...
                ($name -match '^run_bibliometrix(\.exe)?$') -or
                ($RProcessNames -contains $name)
            if ($isOurs) {
...
for ($attempt = 1; $attempt -le 4; $attempt++) {
    ...
    Stop-PortHolders -Ports @(3837, 3838)

    # Give Windows time to release DLL / package file locks under R-Portable.
